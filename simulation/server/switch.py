...
class _Switch(object):
    ...
    def __init__(self, id, num_node=0, num_gpu_p_node=0, num_cpu_p_node=0, mem_p_node=0):
        ...
        self.id = id
        self.node_list = list()
        # print('  Switch[%d] has %d nodes' % (id, num_node))
    ...
    def check_free_gpus(self, ):
        return sum([node.check_free_gpus() for node in self.node_list])
    
    def check_free_cpus(self, ):
        return sum([node.check_free_cpus() for node in self.node_list])

    def check_total_gpus(self, ):
        return sum([node.check_total_gpus() for node in self.node_list])